...
int main(int argc, const char **argv)
{
	...
	lwsl_user("LWS minimal kucoin client\n");
	...
	f = fopen("bullet-public.json", "r");
	if (f && fgets(buf, sizeof(buf), f))
	{
		len = strlen(buf);

		p = lws_json_simple_find((const char *)buf, len, "\"token\":", &alen);
		if (p)
		{
			strncpy(token, p, alen);
		...
		p = lws_json_simple_find((const char *)buf, len, "\"endpoint\":\"wss://", &alen);
		if (p)
		{
			strncpy(address, p, alen - 2);
			lwsl_user("address=%s\n", address);
		}
		else
		{
